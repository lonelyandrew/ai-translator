...
def get_file_downloader_html(file_path: Path, btn_text: str) -> None:
    ...
    # 创建下载链接
    st.markdown(
        f'<a href="data:application/octet-stream;base64,{base64.b64encode(file_content).decode()}" download="{file_path.name}">{btn_text}</a>',
...
# 选择目标语言
target_language: str = st.selectbox("选择目标语言", ["汉语", "法语", "德语", "西班牙语", "日语", "韩语", "粤语"])
...
if uploaded_file is not None:
    ...
    if st.button("开始翻译"):
        with st.spinner(text="翻译中..."):
            with tempfile.NamedTemporaryFile() as temp_pdf_file:
            ...
            for page_idx, page in enumerate(book.pages):
                for content_idx, content in enumerate(page.contents):
                    ...
                    prompt: str = model.translate_prompt(content, target_language)
                    logger.debug(prompt)
                    translation: str
                    status: bool
